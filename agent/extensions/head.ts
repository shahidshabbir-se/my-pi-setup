...
class HeadViewer {
  ...
  private borderWithLabel(
    ...
    if (width <= label.length + 2)
      return this.theme.fg(color, "─".repeat(width));
...
export default function headExtension(pi: ExtensionAPI) {
  pi.registerCommand("head", {
    description:
      "Open the last assistant response from its first line with keyboard scrolling",
    handler: async (_args, ctx) => {
      ...
      await ctx.ui.custom<void>(
        (tui, theme, _keybindings, done) => {
          const container = new HeadViewer(
            found.text,
            found.entryId,
            tui,
            theme,
